...
def prepare_for_denoising(images):
    images = torch.unsqueeze(torch.Tensor(images[0] / 255.), 0)
    images = np.swapaxes(images, 1, 3)  # b, c, w, h
    return images
...
denoiser.load_state_dict(torch.load('Saved_Denoiser/' + '5e-1_VOC_BL3_Denoiser_2' + '.pth'))
denoiser.eval()
...
for i in tqdm(range(0, int(np.ceil(len(ids_test) * 1.0 / batch_size)))):
    with torch.no_grad():
        ids = ids_test[i * batch_size: (i + 1) * batch_size]
        images, masks = get_batch(TASK1_DATA_DIR, TASK1_TRUTH_DIR, ids) # h, w, c   # task1 data dir = training input

        '''
        ############################
        # add noise to clean images
        ISource = prepare_for_denoising(images)
        ...
        for j in range(batch_size):
            if iou_val[j] < 0.65:
                print(ids[j], iou_val[j])
                # print(ids[j], iou_val[j], psnr_test, ssim)
                ############
                # save denoised image
                ############
                cv2.imwrite(str(ids[j]) + "_prediction" + str(iou_val[j]) + ".png", predictions[j] * 255.)
                cv2.imwrite(str(ids[j]) + ".png", images[j])
                cv2.imwrite(str(ids[j]) + "_mask.png", masks[j] * 255.)
            else:
                print(ids[j], score_val[j])
                # print(ids[j], score_val[j], psnr, ssim)
                ############
                # save denoised image
                ############
                cv2.imwrite(str(ids[j]) + "_prediction.png", predictions[j] * 255.)
                cv2.imwrite(str(ids[j]) + ".png", images[j])
                cv2.imwrite(str(ids[j]) + "_mask.png", masks[j] * 255.)
    
    # running_metrics.reset()
# print('PSNR = ', psnr_test / len(ids_test))
# print('SSIM = ', ssim_test / len(ids_test))
